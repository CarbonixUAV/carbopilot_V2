...
const AP_Param::GroupInfo AP_BattMonitor_Params::var_info[] = {
    ...
    // @Bitmask: 0:Ignore DroneCAN SoC, 1:MPPT reports input voltage and current, 2:MPPT Powered off when disarmed, 3:MPPT Powered on when armed, 4:MPPT Powered off at boot, 5:MPPT Powered on at boot, 6:Send resistance compensated voltage to GCS, 7:Allow DroneCAN InfoAux to be from a different CAN node, 8:Aggregate monitor reports minimum voltage instead of average, 9:Aggregate monitor reports maximum temperature instead of average, 10:Aggregate monitor reports minimum temperature instead of average
    // @User: Advanced
